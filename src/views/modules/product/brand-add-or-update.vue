  ...
  data() {
    return {
      ...
      dataRule: {
        ...
        firstLetter: [
          {required: true, message: '首字母不能为空', trigger: 'blur'},//加上这个选项，前面才会展示红星标
          {
            validator: (rule, value, callback) => {
              if (!/^[a-zA-Z]$/.test(value)) {
                callback(new Error('首字母只能为a-z或A-Z，且只能输入一位字母'));
              } else {
                callback();
              }
            }, trigger: 'blur'
        ...
        sort: [
          {required: true, message: '排序不能为空', trigger: 'blur'},//加上这个选项，前面才会展示红星标
          {
            validator: (rule, value, callback) => {
              if (!/^\d+$/.test(value)) {
                callback(new Error('排序只能为大于等于0的整数'));
              } else {
                callback();
              }
            }, trigger: 'blur'
